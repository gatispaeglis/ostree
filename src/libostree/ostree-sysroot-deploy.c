...

{
  ...
  if (dest_initramfs_name)
  ...
  else
    {
      ...
      ostree_prepare_root = g_strdup_printf ("init=/ostree/boot.%d/%s/%s/%d/usr/lib/ostree/ostree-prepare-root /",
                                             new_bootversion, osname, bootcsum,
                                             ostree_deployment_get_bootserial (deployment));
      _ostree_kernel_args_replace_take (kargs, g_steal_pointer (&ostree_prepare_root));
    }
